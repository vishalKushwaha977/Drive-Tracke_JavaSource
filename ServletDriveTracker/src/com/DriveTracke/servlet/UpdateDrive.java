...
public class UpdateDrive extends HttpServlet {
	...
	public UpdateDrive() {
		super();
		System.out.println("Inside UpdateDrive constructor");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		CreateDrive createdriver = mapper.readValue(str1, CreateDrive.class);
		System.out.println(createdriver);

		try {
			Connection con = DAOUtil.dBConnection();
			PreparedStatement p = con
					.prepareStatement("update drive set  COMPANY_NAME =?  ,    NO_OF_RSRCS=?   ,   EXP_REQ=?   "
							+ ",  CTC=? ,  JOIN_CRT=?    , POSITION=?,  EDU_CRT =?  ,  FOLLOWUP=?  ,   BOND =? where DID=?");
			...
			p.setObject(10, createdriver.getId());
